...
const SequencerView: React.FC<SequencerViewProps> = ({ steps, currentStep, stepLength, color, onToggle }) => {
  return (
    <div className="grid grid-cols-4 gap-0.5 md:gap-1 p-0.5 md:p-1 bg-white rounded-lg md:rounded-xl shadow-inner border border-gray-100">
      {Array.from({ length: stepLength }).map((_, i) => (
        <button
          ...
          className={`
            ...
            ${steps[i] ? 'scale-90 shadow-lg' : 'scale-75 opacity-10 md:opacity-20'}
            ${currentStep === i ? 'ring-2 md:ring-4 ring-offset-1 md:ring-offset-2 ring-gray-400 ring-opacity-50 z-10' : ''}
          `}
          style={{ 
